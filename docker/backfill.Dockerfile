...
#
# RESTORED 2026-08-21. Commit 1942a6b3 (2026-01-29) archived the whole `docker/`
# directory as "orphaned". It was not: `bin/scrapers/deploy/deploy_scrapers_backfill_job.sh`
# hard-fails on the absence of this file, so all TWELVE scraper backfill Cloud
# Run jobs (br-rosters, bp-props, odds-api-*, nbac-*, bdl-*, espn-*, bdb-*) have
# been un-rebuildable since that date. Backfill jobs have no Cloud Build trigger,
# so this is their only build path and nothing else covers for it -- which is why
# `br-rosters-backfill` was still serving a 2025-09-18 image in August 2026.
#
# Several OTHER live deploy scripts still reference archived siblings that have
# NOT been restored (docker/analytics-processor.Dockerfile,
# docker/precompute-processor.Dockerfile, docker/predictions-coordinator.Dockerfile,
# docker/freshness.Dockerfile, docker/mlb-*.Dockerfile). Those services do have
# Cloud Build triggers, so their manual scripts are broken but not load-bearing.
#
