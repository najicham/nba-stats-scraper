# predictions/shared/mock_xgboost_model_v2.py

"""
Mock XGBoost Model v2 - Improved Based on Error Analysis

Enhanced version with targeted improvements for:
1. High minutes players (36+ mins) - reduced -12.5 underprediction bias
2. High usage players (30+ usage) - reduced -12.3 underprediction bias
3. Low minutes bench players (<20 mins) - reduced +2-4 overprediction bias

Changes from v1:
- Non-linear minutes boost for 36+ minute games
- Usage-based baseline multiplier for star players
- Stronger mean reversion for bench players
...
class MockXGBoostModel:
    ...
    def _predict_single(self, features: np.ndarray) -> float:
        ...
        # v2 IMPROVEMENT #3: Adjust baseline weights for bench players (<20 mins)
        # Error analysis showed +2-4 overprediction bias for low-minutes players
        if minutes < 20:
            # Bench players: Trust season average more (higher variance game-to-game)
            baseline = (
                points_last_5 * 0.20 +    # Reduced from 0.35 (less weight on recent hot streak)
                points_last_10 * 0.25 +   # Reduced from 0.40
                points_season * 0.55      # Increased from 0.25 (trust long-term average)
            )
        else:
            # Starters/regulars: Normal weighting
            baseline = (
                points_last_5 * 0.35 +
                points_last_10 * 0.40 +
                points_season * 0.25
        ...
        # v2 IMPROVEMENT #2: High usage multiplier
        # Error analysis showed -12.3 underprediction bias for 30+ usage players
        # Star players scale production with usage more than linearly
        if usage_rate >= 30:
            usage_multiplier = 1.0 + ((usage_rate - 30) * 0.015)
            baseline = baseline * usage_multiplier
            # 30 usage = 1.00x (no change)
            # 35 usage = 1.075x (+7.5% boost)
            # 40 usage = 1.15x (+15% boost)
        ...
        # v2 IMPROVEMENT #1: Non-linear boost for extreme minutes
        # Error analysis showed -12.5 underprediction bias for 36+ minute games
        if minutes >= 36:
            # Players in 36+ mins are in high-leverage situations
            # Linear scaling with excess minutes above 36
            excess_minutes = minutes - 36
            minutes_adj = 0.8 + (excess_minutes * 0.35)
            # 36 mins = 0.8 (same as v1)
            # 38 mins = 0.8 + 0.7 = 1.5
            # 40 mins = 0.8 + 1.4 = 2.2
            # 42 mins = 0.8 + 2.1 = 2.9
        elif minutes >= 30:
            ...
            # v2 IMPROVEMENT #3: Stronger penalty for bench minutes
            # Error analysis showed +2-4 overprediction bias
            minutes_adj = -2.5  # Increased penalty from -1.2
        elif minutes < 25:
            minutes_adj = -1.2  # Moderate limited role
        else:
            minutes_adj = 0.0
