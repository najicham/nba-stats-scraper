...
"""
V1.6 Rolling MLB Pitcher Strikeouts Predictor

XGBoost-based prediction system using 35 enhanced features.
Adds rolling Statcast metrics, BettingPros projections, and line-relative features.

Features (35):
- All V1.4 features (25): Rolling stats, season stats, context, opponent, workload
- Season swing metrics (f19): SwStr%, CSW%, Chase%
- Line-relative (f30-f32): K avg vs line, projected vs line, line level
- BettingPros (f40-f44): Projection, diff, performance percentiles, implied prob
- Rolling Statcast (f50-f53): SwStr% last 3, FB velocity, SwStr% trend, velocity change
...
# Default values for V1.6 features (extends V1.4)
FEATURE_DEFAULTS_V1_6 = {
...
# Mapping from raw feature names to model feature names (V1.6 extends V1.4)
RAW_TO_MODEL_MAPPING_V1_6 = {
    # V1.4 mappings
...
class V1_6RollingPredictor(BaseMLBPredictor):
    """
    V1.6 Rolling XGBoost predictor for pitcher strikeouts

    Uses 35 enhanced features including rolling Statcast metrics,
    BettingPros projections, and line-relative features.

    Default model: mlb_pitcher_strikeouts_v1_6_rolling_20260115_131149.json
    """
    ...
    def __init__(
        ...
        """
        Initialize V1.6 rolling predictor

        Args:
            model_path: GCS path to model (default: V1.6 model)
            project_id: GCP project ID
        """
        super().__init__(system_id='v1_6_rolling', project_id=project_id)

        # Default to V1.6 model
        default_model = 'gs://nba-scraped-data/ml-models/mlb/mlb_pitcher_strikeouts_v1_6_rolling_20260115_131149.json'
        self.model_path = model_path or os.environ.get('MLB_V1_6_MODEL_PATH', default_model)
        self.model = None
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            local_path = '/tmp/mlb_v1_6_rolling_model.json'
            ...
            metadata_local = '/tmp/mlb_v1_6_rolling_metadata.json'
    ...
    def prepare_features(self, raw_features: Dict) -> Optional[np.ndarray]:
        """
        Prepare feature vector from raw features for V1.6 model
        ...
        try:
            # Normalize raw_features to model feature names
            normalized_features = {}
            ...
            for key, value in raw_features.items():
                ...
                # Otherwise, try to map it using RAW_TO_MODEL_MAPPING_V1_6
                elif key in RAW_TO_MODEL_MAPPING_V1_6:
                    model_key = RAW_TO_MODEL_MAPPING_V1_6[key]
            ...
            for feature_name in self.feature_order:
                ...
                if value is None:
                    value = FEATURE_DEFAULTS_V1_6.get(feature_name, 0.0)
            ...
            if np.any(np.isnan(result)) or np.any(np.isinf(result)):
                ...
                for i, val in enumerate(result[0]):
                    if np.isnan(val) or np.isinf(val):
                        result[0][i] = FEATURE_DEFAULTS_V1_6.get(self.feature_order[i], 0.0)
    ...
    def predict(
        ...
        """
        Generate strikeout prediction for a pitcher using V1.6 rolling model
        ...
        try:
            ...
            # V1.6 is a regressor (outputs strikeout count directly)
