# predictions/mlb/prediction_systems/lightgbm_v1_regressor_predictor.py
"""
LightGBM V1 MLB Pitcher Strikeouts Regressor Predictor

Same 36-feature contract as CatBoost V2 regressor. Predicts raw strikeout
count. Edge is in real K units (predicted - line).

System ID: 'lightgbm_v1_regressor'
"""
...
class LightGBMV1RegressorPredictor(BaseMLBPredictor):
    """LightGBM V1 regressor for pitcher strikeout predictions.

    Uses identical feature contract as CatBoost V2 regressor (36 features).
    LightGBM handles NaN natively for NaN-tolerant features.
    """

    def __init__(self, model_path: str = None, project_id: str = None):
        super().__init__(system_id='lightgbm_v1_regressor', project_id=project_id)

        default_model = 'gs://nba-props-platform-ml-models/mlb/lightgbm_mlb_v1_regressor_36f.txt'
        self.model_path = model_path or os.environ.get('MLB_LIGHTGBM_V1_MODEL_PATH', default_model)
        self.model = None
    ...
    def load_model(self) -> bool:
        ...
        try:
            ...
            local_path = '/tmp/mlb_lightgbm_v1_regressor_model.txt'
            blob.download_to_filename(local_path)

            self.model = lgb.Booster(model_file=local_path)

            # Load metadata
            metadata_blob_path = blob_path.replace('.txt', '_metadata.json')
            metadata_blob = bucket.blob(metadata_blob_path)
            metadata_local = '/tmp/mlb_lightgbm_v1_regressor_metadata.json'
    ...
    def predict(self, pitcher_lookup: str, features: Dict,
        ...
        """Generate strikeout prediction using LightGBM V1 regressor."""
        ...
        try:
            ...
            return {
                ...
                'strikeouts_line': strikeouts_line,
                'system_id': self.system_id,
                'model_version': 'lightgbm_v1_regressor',
                'p_over': round(p_over, 4),
