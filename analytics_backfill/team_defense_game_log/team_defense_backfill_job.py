#!/usr/bin/env python3
"""
File: analytics_backfill/team_defense_game_log/team_defense_backfill_job.py

Backfill job for team defense analytics.
Processes date ranges from raw BigQuery tables into team defense analytics tables.
...
from analytics_processors.team_defense_game_log.team_defense_processor import TeamDefenseProcessor
...
class TeamDefenseBackfill:
    """Backfill team defense analytics data."""
    
    def __init__(self):
        self.processor = TeamDefenseProcessor()
    ...
    def run_backfill(self, start_date: date, end_date: date, 
        ...
        """
        logger.info(f"Starting team defense analytics backfill from {start_date} to {end_date}")
        ...
        for i, (chunk_start, chunk_end) in enumerate(chunks, 1):
            try:
                ...
                if success:
                    ...
                    chunk_records = stats.get('defensive_games_processed', 0)
                    total_records += chunk_records
                    successful_chunks += 1
                    
                    logger.info(f"Chunk {i} completed: {chunk_records} defensive games processed")
                else:
        ...
        # Summary
        logger.info("=" * 50)
        logger.info("TEAM DEFENSE ANALYTICS BACKFILL COMPLETE")
        logger.info(f"Successful chunks: {successful_chunks}/{len(chunks)}")
        logger.info(f"Total defensive games processed: {total_records}")
        logger.info(f"Date range: {start_date} to {end_date}")
        logger.info("=" * 50)

def main():
    parser = argparse.ArgumentParser(description='Backfill team defense analytics')
    ...
    # Run backfill
    backfiller = TeamDefenseBackfill()
