...
SELECT
  ...
  CASE
    -- Off day scenarios
    WHEN s.scheduled_games = 0 AND r.hourly_snapshots IS NULL 
      THEN '⚪ No games scheduled, no reports (expected)'
    WHEN s.scheduled_games = 0 AND r.hourly_snapshots > 0 
      THEN '✅ No games scheduled, reports exist'
