...
| **Retrain gate backwards (Session 483)** | `weekly-retrain` CF gate pauses when MAE > 5.0. This is WRONG — MAE 5.43 (LOOSE) is best time to train. Gate should block training ON tight data, not block training DURING loose market. Use `--train-end 2026-02-28` to pin to pre-tightening data. |
| **MLB pitcher-props-validator was April-only** | Schedule was `4-10` (April-October). Fixed to `3-10` (March-October) — MLB Opening Day is late March. If season start shifts, update `deployment/scheduler/mlb/validator-schedules.yaml`. |
