...
| **Orchestrator not triggering** | **Phase 2 complete but `_triggered=False`** | **CRITICAL (Session 205): Missing IAM permissions. All orchestrators need `roles/run.invoker` for service account. Fix: `gcloud run services add-iam-policy-binding <orchestrator> --region=us-west2 --member='serviceAccount:756957797294-compute@developer.gserviceaccount.com' --role='roles/run.invoker'`. Manual trigger if needed: `gcloud scheduler jobs run same-day-phase3 --location=us-west2`. Historical issue (Session 197): BDL dependencies - now fixed.** |
