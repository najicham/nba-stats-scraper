...
| **Prediction requests land in `prediction-request-dlq`** | `prediction-worker` is `maxScale=10`, `containerConcurrency=1`; Pub/Sub push delivers a whole slate at once and Cloud Run returns "no available instance". Measured 2026-09-07: **107 of 399 requests (27%) dead-lettered in under 2 minutes**. Nothing consumes that DLQ. Raise maxScale and/or lengthen the retry backoff before a full slate. |
