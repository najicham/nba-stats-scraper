...
gcloud run jobs create $JOB_NAME \
    ...
    --max-retries 2 \
    --task-timeout 5m \
    --update-env-vars "GCP_PROJECT_ID=$PROJECT_ID,SLACK_WEBHOOK_URL_WARNING=$(gcloud secrets versions access latest --secret="slack-webhook-warning" --project=$PROJECT_ID 2>/dev/null || echo "")" \
