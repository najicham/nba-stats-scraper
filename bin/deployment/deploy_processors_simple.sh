#!/bin/bash
# deploy_processors_simple.sh - Deploy processor service to Cloud Run
#
# Similar to deploy_scrapers_simple.sh but for processors
#
# WHAT THIS DOES:
# 1. Copies processors/Dockerfile to root temporarily
# 2. Deploys using `gcloud run deploy --source=.` 
# 3. Cleans up temporary Dockerfile
# 4. Tests the health endpoint
#
# USAGE: ./bin/deployment/deploy_processors_simple.sh
...
echo "🚀 Deploying NBA Processors Service"
echo "===================================="
echo "⏰ Start time: $DEPLOY_START_DISPLAY"

# Check if processors/Dockerfile exists
if [ ! -f "processors/Dockerfile" ]; then
    echo "❌ processors/Dockerfile not found!"
    exit 1
fi
...
# Phase 1: Setup
SETUP_START=$(date +%s)
echo "📋 Phase 1: Copying processors/Dockerfile to root..."
cp processors/Dockerfile ./Dockerfile
SETUP_END=$(date +%s)
