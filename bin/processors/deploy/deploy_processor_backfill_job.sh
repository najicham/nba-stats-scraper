#!/bin/bash
# File: bin/deployment/deploy_processor_backfill_job.sh
#
...
if [[ -z "$1" ]]; then
    ...
    echo "  $0 odds_api_props              # Auto-finds processor_backfill/odds_api_props/job-config.env"
    echo "  $0 odds-api-props-backfill      # Auto-finds based on job name"
    echo "  $0 br_roster                    # Auto-finds processor_backfill/br_roster/job-config.env"
    echo ""
    echo "Examples (explicit path - still supported):"
    echo "  $0 processor_backfill/odds_api_props/job-config.env"
    echo ""
    echo "Available processor jobs:"
    find processor_backfill/ -name "job-config.env" 2>/dev/null | sed 's|processor_backfill/||' | sed 's|/job-config.env||' | sed 's/^/  /' || echo "  No jobs found yet"
    exit 1
fi
...
discover_config_file() {
    ...
    local patterns=(
        "processor_backfill/${input}/job-config.env"                    # odds_api_props → processor_backfill/odds_api_props/job-config.env
        "processor_backfill/${input/_/-}/job-config.env"                # odds_api_props → processor_backfill/odds-api-props/job-config.env  
        "processor_backfill/${input//-/_}/job-config.env"               # odds-api-props → processor_backfill/odds_api_props/job-config.env
        "processor_backfill/${input%-backfill}/job-config.env"          # odds-api-props-backfill → processor_backfill/odds-api-props/job-config.env
        "processor_backfill/${input%-backfill}/job-config.env"          # odds_api_props_backfill → processor_backfill/odds_api_props/job-config.env
    )
...
if [[ -z "$CONFIG_FILE" ]]; then
    ...
    echo "  processor_backfill/${JOB_INPUT}/job-config.env"
    echo "  processor_backfill/${JOB_INPUT/_/-}/job-config.env"
    echo "  processor_backfill/${JOB_INPUT//-/_}/job-config.env"
    echo ""
    echo "Available jobs:"
    find processor_backfill/ -name "job-config.env" 2>/dev/null | sed 's|processor_backfill/||' | sed 's|/job-config.env||' | sed 's/^/  /' || echo "  No jobs found yet"
    ...
    echo "   $0 processor_backfill/your-job/job-config.env"
    exit 1
fi
