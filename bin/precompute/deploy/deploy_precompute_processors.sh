...
if [[ -n "${EMAIL_ALERTS_TO:-}" ]]; then
    ...
    ENV_VARS="$ENV_VARS,AWS_SES_FROM_NAME=${AWS_SES_FROM_NAME:-NBA Precompute System}"
    ...
    ENV_VARS="$ENV_VARS,BREVO_FROM_NAME=${BREVO_FROM_NAME:-NBA Precompute System}"
...
# Check if precompute processors Dockerfile exists
if [ ! -f "docker/precompute-processor.Dockerfile" ]; then
    echo "❌ docker/precompute-processor.Dockerfile not found!"
    exit 1
fi
...
# Phase 2: Setup
SETUP_START=$(date +%s)
echo "📋 Phase 2: Copying docker/precompute-processor.Dockerfile to root..."
...
if [ $DEPLOY_STATUS -eq 0 ]; then
    ...
    if [ ! -z "$SERVICE_URL" ]; then
        ...
        # Instructions for precompute processing
        echo ""
        echo "📝 Next Steps - Precompute Processing Commands:"
        echo "==============================================="
        echo "1. Team defense zone analysis:"
        echo "   curl -X POST \"$SERVICE_URL/process-precompute\" \\"
        echo "     -H \"Authorization: Bearer \$(gcloud auth print-identity-token)\" \\"
        echo "     -H \"Content-Type: application/json\" \\"
        echo "     -d '{\"processor\": \"team_defense_zone_analysis\", \"analysis_date\": \"2024-11-22\"}'"
        echo ""
        echo "2. Player shot zone analysis:"
        echo "   curl -X POST \"$SERVICE_URL/process-precompute\" \\"
        echo "     -H \"Authorization: Bearer \$(gcloud auth print-identity-token)\" \\"
        echo "     -H \"Content-Type: application/json\" \\"
        echo "     -d '{\"processor\": \"player_shot_zone_analysis\", \"analysis_date\": \"2024-11-22\"}'"
        echo ""
        echo "3. Player daily cache:"
        echo "   curl -X POST \"$SERVICE_URL/process-precompute\" \\"
        echo "     -H \"Authorization: Bearer \$(gcloud auth print-identity-token)\" \\"
        echo "     -H \"Content-Type: application/json\" \\"
        echo "     -d '{\"processor\": \"player_daily_cache\", \"analysis_date\": \"2024-11-22\"}'"
        echo ""
        echo "4. Player composite factors (CASCADE):"
        echo "   curl -X POST \"$SERVICE_URL/process-precompute\" \\"
        echo "     -H \"Authorization: Bearer \$(gcloud auth print-identity-token)\" \\"
        echo "     -H \"Content-Type: application/json\" \\"
        echo "     -d '{\"processor\": \"player_composite_factors\", \"analysis_date\": \"2024-11-22\"}'"
        echo ""
        echo "5. ML feature store (CASCADE):"
        echo "   curl -X POST \"$SERVICE_URL/process-precompute\" \\"
        echo "     -H \"Authorization: Bearer \$(gcloud auth print-identity-token)\" \\"
        echo "     -H \"Content-Type: application/json\" \\"
        echo "     -d '{\"processor\": \"ml_feature_store\", \"analysis_date\": \"2024-11-22\"}'"
    fi
else
    echo ""
