...
SELECT
  ...
  ROUND(100.0 * COUNTIF(feature_count = 54) / COUNT(*), 1) as pct_54_features,
  ROUND(100.0 * COUNTIF(feature_quality_score >= 70) / COUNT(*), 1) as pct_quality_ok,
  ROUND(100.0 * COUNTIF(feature_33_value IS NOT NULL) / COUNT(*), 1) as pct_has_trajectory,

  -- Alert flags
  CASE
    WHEN COUNTIF(feature_count = 54) < COUNT(*) * 0.95 THEN 'SCHEMA_INCOMPLETE'
