...
| Metric | Value |
|--------|-------|
| Model ID | xgboost_v1_33features_20260117_183235 |
| System ID | `xgboost_v1` |
| Training Samples | 115,333 |
| Training Date Range | 2021-11-02 to 2025-12-31 |
...
|--------|-------|--------|--------|
| MAE | 3.98 points | ≤ 4.5 | ✅ Beat target |
| RMSE | 5.59 points | - | - |
| Within 3 pts | 53.0% | - | - |
| Within 5 pts | 72.8% | - | - |
| vs Mock V1 | +17.1% better | - | ✅ |
| vs CatBoost V8 | -17.0% worse | - | Competitive |
...
**Best Iteration:** 521 (out of 1000 max)
...
| 11 | minutes_change | 1.0% | Base |
| 12 | vegas_line_move | 1.0% | Vegas |
| 13 | rest_advantage | 0.9% | Base |
| 14 | fatigue_score | 0.8% | Base |
| 15 | opponent_def_rating | 0.8% | Base |

**Key Insights:**
- Recent performance (last 5-10 games): **54.7%** combined
- Vegas features: **23.4%** combined (market efficiency)
- Minutes/PPM context: **3.2%** combined
...
FROM \`nba-props-platform.nba_predictions.prediction_accuracy\`
WHERE game_date >= '2026-01-17'  -- Deployment date
  AND system_id = 'xgboost_v1'
  AND recommendation IN ('OVER', 'UNDER')
  AND has_prop_line = TRUE
```

**Expected Production MAE:** ~3.98 points (based on validation)
...
If production MAE > 4.5 (significantly worse than validation 3.98):
...
Expected: `gs://nba-scraped-data/ml-models/xgboost_v1_33features_20260117_183235.json`
...
1. **Compare to validation baseline:** Production MAE should be ~3.98 ± 0.5
...
1. **Retrain evaluation:** Compare MAE to training (3.48) and validation (3.98)
...
# - Check validation MAE (should be ≤ 3.98 or better)
...
- **Model Metadata:** `models/xgboost_v1_33features_20260117_183235_metadata.json`
- **Deployment Guide:** `docs/09-handoff/SESSION-93-COMPLETE.md`
...
### Validation Set Performance (Training)

| Model | MAE | RMSE | Within 3 pts | Within 5 pts |
|-------|-----|------|--------------|--------------|
| Mock XGBoost V1 | 4.80 | - | - | - |
| **XGBoost V1** | **3.98** | 5.59 | 53.0% | 72.8% |
| CatBoost V8 | 3.40 | - | - | - |
...
**Model Version:** xgboost_v1_33features_20260117_183235
**Deployment:** Production (2026-01-17 18:43 UTC)
