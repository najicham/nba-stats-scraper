...
**Updated:** 2026-02-15 (Session 267 — consolidated review)
...
### Priority 2: Backfill Best Bets + Signal Data
- Backfill best_bets subset (id=26) into `current_subset_picks` for Jan 9 - Feb 18 (enables grading history)
- Backfill `signal-best-bets/{date}.json` GCS files for website track record
- See `SESSION-267-FORWARD-PLAN.md` Section 5 for commands

### Priority 3: New Phase 6 Exports (Website)
- Build `v1/systems/signal-health.json` export (per-signal HOT/COLD/NORMAL regime)
- Build `v1/systems/model-health.json` export (model state + "sitting out" messaging)
- Optional: `v1/systems/combo-registry.json` for combo explanations

### Priority 4: Post-Break Validation
