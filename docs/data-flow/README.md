...
**Status:** 📚 Building - Phase 1 scraper outputs documented
...
│ Tables: 7 analytics tables (game summaries, context, trends)    │
│ Aggregation: Rolling windows, advanced metrics                  │
│ Status: 2 blocked ⚠️, 1 in progress 🟡, 4 ready 🟢             │
└──────────────────────────────────────────────────────────────────┘
                            ↓
                    [5 Processors]
...
│ Status: 4 ready 🟢, 1 blocked ⚠️ (ml_feature_store_v2)         │
└──────────────────────────────────────────────────────────────────┘
                            ↓
...
│ Status: Blocked ⚠️ (awaiting ml_feature_store_v2)               │
└──────────────────────────────────────────────────────────────────┘
                            ↓
                    [Future: Phase 6]
                            ↓
...
|-------|-----------|--------------|---------------|--------------|--------|
| 1→2 | 21 | 26 scrapers | 21 raw tables | ~50 JSON files | 🟢 Complete |
| 2→3 | 7 | 21 raw tables | 7 analytics tables | ~2,000 rows | 🟡 Partial |
| 3→4 | 5 | 7 analytics tables | 5 precompute tables | ~1,350 rows | ⚠️ 1 blocked |
| 4→5 | 5 systems | 1 feature store | 1 predictions table | ~2,250 rows | ⚠️ Blocked |
...
| **03** | Team Offense | 2→3 | nbac_team_boxscore | team_offense_game_summary | ⚠️ | Raw table missing |
| **04** | Team Defense | 2→3 | nbac_team_boxscore | team_defense_game_summary | ⚠️ | Raw table missing |
| **05** | Upcoming Team Context | 2→3 | 5 raw tables | upcoming_team_game_context | 🟡 | Source tracking incomplete |
| **06** | Upcoming Player Context | 2→3 | 8 raw tables | upcoming_player_game_context | ⚠️ | 2 raw tables missing |
...
| **12** | ML Feature Store | 3→4 | 3 analytics + 4 phase4 | ml_feature_store_v2 | ⚠️ | Output table missing |
| **13** | Feature Consumption | 4→5 | ml_feature_store_v2 | player_prop_predictions | ⚠️ | Source table missing |

**Legend:**
- 🟢 Production Ready - All sources available, processor implemented, output table exists
- 🟡 In Progress - Implementation incomplete or source tracking missing
- ⚠️ Blocked - Critical dependency missing (table, processor, or data)

### Deployment Status Summary

**Ready for Production (6 docs):**
- Doc 01: Scraper catalog
- Doc 02: Phase 1→2 processors
- Doc 07: Player game summary
- Docs 08-11: Phase 4 zone analysis, cache, composite factors

**Blocked on Phase 2 Deployment (3 docs):**
- Doc 03: Team offense (needs nbac_team_boxscore)
- Doc 04: Team defense (needs nbac_team_boxscore)
- Doc 06: Upcoming player context (needs odds_player_props + nbac_schedule)

**In Progress (1 doc):**
- Doc 05: Upcoming team context (source tracking incomplete)

**Blocked on Phase 4 Deployment (2 docs):**
- Doc 12: ML Feature Store (table not created)
- Doc 13: Feature consumption (depends on Doc 12)

**Critical Path to Phase 5:**
```
1. Deploy Phase 2 raw tables (nbac_team_boxscore, odds_player_props) → Unblocks Docs 03-06
2. Create ml_feature_store_v2 table → Unblocks Docs 12-13
3. Phase 5 predictions can run
```
...
**Directory Status:** 📚 Building (Phase 1→2 complete, Phase 2→3 documented, Phase 3→4 complete, Phase 4→5 started)
