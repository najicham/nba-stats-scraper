#!/usr/bin/env python3
# FILE: backfill_jobs/scrapers/nbac_injury/nbac_injury_backfill_job.py
# MODIFIED TO ADD --dates PARAMETER FOR TARGETED BACKFILLS
...
NEW: Supports --dates parameter for targeted backfills of specific dates.
...
Usage:
  ...
  ./backfill_jobs/scrapers/nbac_injury/deploy.sh
  ...
  # NEW: Specific dates (All-Star weekend backfill):
  gcloud run jobs execute nbac-injury-backfill \
    --args="--dates=2022-02-18,2022-02-20,2023-02-17,2023-02-19,2024-02-16,2024-02-18,2025-02-14,2025-02-16" \
    --region=us-west2
...
class NbaInjuryBackfillJob:
    """Cloud Run Job for downloading NBA injury report PDFs using hourly interval strategy."""
    
    def __init__(self, scraper_service_url: str, seasons: List[int] = None, bucket_name: str = "nba-scraped-data", 
                 limit: Optional[int] = None, start_date: str = None, specific_dates: List[str] = None):
        self.scraper_service_url = scraper_service_url.rstrip('/')
        self.seasons = seasons or [2021, 2022, 2023, 2024]  # Default: all 4 seasons
        self.bucket_name = bucket_name
        self.limit = limit
        self.start_date = start_date
        self.specific_dates = specific_dates  # NEW: List of specific dates to process
        ...
        if self.specific_dates:
            logger.info("Mode: TARGETED DATES")
            logger.info("Specific dates: %s", self.specific_dates)
        else:
            logger.info("Mode: SEASON-BASED")
    ...
    def run(self, dry_run: bool = False):
        ...
        try:
            # 1. Collect all game dates - NEW: use specific dates if provided
            if self.specific_dates:
                all_game_dates = self._validate_specific_dates()
            else:
    ...
    def _validate_specific_dates(self) -> List[str]:
        """
        NEW: Validate and return specific dates provided via --dates parameter.
        No need to read from schedule - just validate format.
        """
        logger.info("📊 Validating specific dates...")
        ...
        logger.info(f"🎯 Total specific dates to process: {len(validated_dates)}")
        
        if validated_dates:
            logger.info(f"Date range: {validated_dates[0]} to {validated_dates[-1]}")
...
def main():
    ...
    parser.add_argument("--dates",
                       help="NEW: Comma-separated specific dates (YYYY-MM-DD format) for targeted backfill")
    ...
    # Parse seasons list (if no specific dates provided)
    seasons = None
    specific_dates = None
    
    if args.dates:
        # NEW: Use specific dates mode
        specific_dates = [d.strip() for d in args.dates.split(",")]
        logger.info("Using specific dates mode: %d dates", len(specific_dates))
    else:
        # Traditional season-based mode
    ...
    job = NbaInjuryBackfillJob(
        ...
        specific_dates=specific_dates  # NEW
    )
