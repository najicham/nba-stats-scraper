...
-- Description: BigQuery table schema for NBA.com team box score data
--
...
CREATE TABLE IF NOT EXISTS `nba_raw.nbac_team_boxscore` (
  ...
  game_id              STRING NOT NULL,    -- NBA.com game ID: "0022400561"
...
CLUSTER BY game_id, team_abbr, season_year
OPTIONS (
  description = "NBA.com team box score statistics - complete team-level performance data per game. Each game produces exactly 2 rows (one per team). Updated via MERGE_UPDATE strategy.",
...
-- GROUP BY t.game_date, t.game_id, t.team_abbr, t.points
...
-- Check 2: Verify FG math (made ≤ attempted)
-- SELECT game_id, game_date, team_abbr, fg_made, fg_attempted
-- FROM `nba_raw.nbac_team_boxscore`
-- WHERE fg_made > fg_attempted
--   AND game_date >= '2025-01-01';

-- Check 3: Verify 3PT math (3PT made ≤ FG made)
-- SELECT game_id, game_date, team_abbr, three_pt_made, fg_made
-- FROM `nba_raw.nbac_team_boxscore`
-- WHERE three_pt_made > fg_made
--   AND game_date >= '2025-01-01';

-- Check 4: Verify rebound math (offensive + defensive = total)
...
-- Check 5: Verify points calculation
-- SELECT game_id, game_date, team_abbr, points,
...
-- Check 6: Verify plus/minus sums to zero per game
