"""
FantasyPros Projections Processor

GCS Path: gs://nba-scraped-data/projections/fantasypros/{date}/{timestamp}.json
...
class FantasyProsProjectionsProcessor(SmartIdempotencyMixin, ProcessorBase):
    """
    FantasyPros Projections Processor
    ...
    def __init__(self):
        super().__init__()
        self.table_name = 'nba_raw.fantasypros_projections'
    ...
    def transform_data(self) -> None:
        """Transform FantasyPros JSON into BQ rows."""
        ...
        try:
            ...
            if not game_date:
                raise ValueError(f"Missing date in FantasyPros data: {file_path}")
            ...
            logger.info(f"Transformed {len(rows)} FantasyPros projections from {file_path}")

        except Exception as e:
            logger.error(f"Transform failed for {file_path}: {e}", exc_info=True)
            try:
                notify_error(
                    title="FantasyPros Projections Transform Failed",
                    message=str(e),
    ...
    def save_data(self) -> None:
        ...
        try:
            ...
            if load_job.errors:
                ...
                logger.info(f"Loaded {len(rows)} FantasyPros projection rows to BQ")
            ...
            try:
                notify_error(
                    title="FantasyPros BQ Load Failed",
                    message=str(e),
