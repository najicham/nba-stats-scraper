"""
ESPN Projections Processor

GCS Path: gs://nba-scraped-data/projections/espn/{date}/{timestamp}.json
BigQuery Table: nba_raw.espn_projections
...
class ESPNProjectionsProcessor(SmartIdempotencyMixin, ProcessorBase):
    """
    ESPN Projections Processor
    ...
    def __init__(self):
        super().__init__()
        self.table_name = 'nba_raw.espn_projections'
    ...
    def transform_data(self) -> None:
        """Transform ESPN JSON into BQ rows."""
        ...
        try:
            ...
            if not game_date:
                raise ValueError(f"Missing date in ESPN data: {file_path}")
            ...
            logger.info(f"Transformed {len(rows)} ESPN projections from {file_path}")

        except Exception as e:
            logger.error(f"Transform failed for {file_path}: {e}", exc_info=True)
            try:
                notify_error(
                    title="ESPN Projections Transform Failed",
                    message=str(e),
    ...
    def save_data(self) -> None:
        ...
        try:
            ...
            if load_job.errors:
                ...
                logger.info(f"Loaded {len(rows)} ESPN projection rows to BQ")
