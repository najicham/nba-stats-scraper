"""
DailyFantasyFuel Projections Processor

GCS Path: gs://nba-scraped-data/projections/dailyfantasyfuel/{date}/{timestamp}.json
...
class DailyFantasyFuelProjectionsProcessor(SmartIdempotencyMixin, ProcessorBase):

    HASH_FIELDS = ['player_lookup', 'game_date', 'projected_points']

    def __init__(self):
        super().__init__()
        self.table_name = 'nba_raw.dailyfantasyfuel_projections'
    ...
    def transform_data(self) -> None:
        ...
        try:
            ...
            logger.info(f"Transformed {len(rows)} DFF projections from {file_path}")
        except Exception as e:
            logger.error(f"Transform failed for {file_path}: {e}", exc_info=True)
            try:
                notify_error(title="DFF Transform Failed", message=str(e),
