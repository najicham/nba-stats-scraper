"""
Dimers Projections Processor

GCS Path: gs://nba-scraped-data/projections/dimers/{date}/{timestamp}.json
...
class DimersProjectionsProcessor(SmartIdempotencyMixin, ProcessorBase):

    HASH_FIELDS = ['player_lookup', 'game_date', 'projected_points']

    def __init__(self):
        super().__init__()
        self.table_name = 'nba_raw.dimers_projections'
    ...
    def transform_data(self) -> None:
        ...
        try:
            ...
            logger.info(f"Transformed {len(rows)} Dimers projections from {file_path}")
        except Exception as e:
            logger.error(f"Transform failed for {file_path}: {e}", exc_info=True)
            try:
                notify_error(title="Dimers Transform Failed", message=str(e),
