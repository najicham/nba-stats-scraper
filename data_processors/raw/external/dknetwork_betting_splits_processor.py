"""
DraftKings Network Betting Splits Processor

GCS Path: gs://nba-scraped-data/external/dknetwork/betting-splits/{date}/{timestamp}.json
BigQuery Table: nba_raw.dknetwork_betting_splits

Free DraftKings-sourced replacement for the paywalled VSiN source. Same game-total
betting splits (over/under ticket% + money%), plus DK event id and odds.
"""
...
class DKNetworkBettingSplitsProcessor(SmartIdempotencyMixin, ProcessorBase):

    HASH_FIELDS = ['away_team', 'home_team', 'game_date', 'over_ticket_pct']

    def __init__(self):
        super().__init__()
        self.table_name = 'nba_raw.dknetwork_betting_splits'
        ...
        try:
            ...
            for game in raw_data.get('games', []):
                row = {
                    ...
                    'dk_event_id': game.get('dk_event_id'),
            ...
            logger.info(f"Transformed {len(rows)} DK Network betting splits from {file_path}")
        except Exception as e:
            logger.error(f"Transform failed for {file_path}: {e}", exc_info=True)
            try:
                notify_error(title="DK Network Transform Failed", message=str(e),
