...
class TonightAllPlayersExporter(BaseExporter):
    ...
    def _query_players(self, target_date: str) -> List[Dict]:
        ...
        query = """
        WITH predictions AS (
            -- Get best active prediction per player (multi-model aggregator selects winner)
            -- Use ROW_NUMBER to deduplicate in case of multiple active rows per player/game
            SELECT
                pp.player_lookup,
                pp.game_id,
                pp.game_date,
