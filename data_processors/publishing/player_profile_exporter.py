    ...
    def _query_game_log(self, player_lookup: str, limit: int = 50) -> List[Dict]:
        ...
        for r in results:
            formatted.append({
                ...
                # Separate makes/attempts for frontend (also keep combined string for backwards compat)
                'fg': f"{r['fg_makes']}/{r['fg_attempts']}" if r.get('fg_attempts') else None,
                'fg_makes': r.get('fg_makes'),
                'fg_attempts': r.get('fg_attempts'),
                'three': f"{r['three_pt_makes']}/{r['three_pt_attempts']}" if r.get('three_pt_attempts') else None,
                'three_makes': r.get('three_pt_makes'),
                'three_attempts': r.get('three_pt_attempts'),
                'ft': f"{r['ft_makes']}/{r['ft_attempts']}" if r.get('ft_attempts') else None,
                'ft_makes': r.get('ft_makes'),
                'ft_attempts': r.get('ft_attempts'),
