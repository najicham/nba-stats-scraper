...
class FeatureCalculator:
    ...
    def calculate_recent_trend(self, phase3_data: Dict) -> float:
        ...
        # Take 5 most recent games. last_10_games is ordered newest-first
        # (feature_extractor SQL: ORDER BY game_date DESC), so index 0 is the
        # most recent game and higher indices are older games.
        last_5_games = last_10_games[:5]

        # Split into windows (handle None values).
        # Newest-first ordering: [0:2] = 2 most recent games, [2:5] = 3 older games.
        recent_2_points = [g.get('points') or 0 for g in last_5_games[0:2]]
        older_3_points = [g.get('points') or 0 for g in last_5_games[2:5]]

        # Calculate averages
        avg_last_2 = sum(recent_2_points) / 2.0
        avg_first_3 = sum(older_3_points) / 3.0

        # Calculate difference: positive = recent > older = improving
