#!/usr/bin/env python3
"""
MLB Odds API Historical Batter Props Scraper

Fetches historical batter prop odds from The Odds API at a specific snapshot.
Critical for training the bottom-up K prediction model on historical data.

Endpoint: GET /v4/historical/sports/baseball_mlb/events/{eventId}/odds

Usage:
  # Get historical batter props for a specific event:
  SPORT=mlb python scrapers/mlb/oddsapi/mlb_batter_props_his.py \
      --event_id abc123 --game_date 2025-06-15 \
      --snapshot_timestamp 2025-06-15T18:00:00Z --group dev

  # Flask service:
  SPORT=mlb python scrapers/mlb/oddsapi/mlb_batter_props_his.py --serve --debug
...
class MlbBatterPropsHistoricalScraper(ScraperBase, ScraperFlaskMixin):
    """
    MLB Historical Batter Props Scraper for The Odds API.

    Fetches batter prop betting lines at a specific historical snapshot.
    Batter K lines are critical for the bottom-up prediction model.
    ...
    scraper_name = "mlb_batter_props_his"
    ...
    GCS_PATH_KEY = "mlb_odds_api_batter_props_history"
    exporters = [
        {
            "type": "gcs",
        ...
        {
            "type": "file",
            "filename": "/tmp/mlb_batter_props_his_%(event_id)s.json",
            "pretty_print": True,
    ...
    def set_url(self) -> None:
        ...
        logger.info("MLB Historical Batter Props URL: %s", self.url.replace(api_key, "***"))
    ...
    def transform_data(self) -> None:
                ...
                if mkt.get("key") == "batter_strikeouts":
                    for outcome in outcomes:
                        if outcome.get("name") == "Over":
                            point = outcome.get("point", 0) or 0
        ...
        if row_count == 0:
            notify_warning(
                title="No MLB Historical Batter Props",
                message=f"No batter props at snapshot for event {self.opts.get('event_id')}",
                details={
                    'scraper': 'mlb_batter_props_his',
            ...
            notify_info(
                title="MLB Historical Batter Props Retrieved",
                message=f"Found {len(strikeout_lines)} batter K lines (~{total_implied_ks:.1f} implied K's)",
                details={
                    'scraper': 'mlb_batter_props_his',
                    'event_id': self.opts.get('event_id'),
                    'game_date': self.opts.get('game_date'),
        ...
        logger.info("Fetched %d historical batter prop outcomes (%d strikeout lines, ~%.1f implied K's)",
                   row_count, len(strikeout_lines), total_implied_ks)
...
create_app = convert_existing_flask_scraper(MlbBatterPropsHistoricalScraper)

if __name__ == "__main__":
    main = MlbBatterPropsHistoricalScraper.create_cli_and_flask_main()
    main()
