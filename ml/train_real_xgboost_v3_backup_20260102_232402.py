...
SELECT
  ...
  -- Placeholder features for future implementation (4) - indices 9-12
  0.0 as referee_favorability_score,
  0.0 as look_ahead_pressure_score,
  0.0 as matchup_history_score,
  0.0 as momentum_score,

  -- Opponent defense metrics from precompute (2) - indices 13-14
  ...
  -- Game context features (3) - indices 15-17
...
print(f"\n🎯 COMPLETE MODEL: Using {len(feature_cols)} features")
print(f"   - 14 from v2 model")
print(f"   - 7 new context/opponent/team features")
print(f"   - 4 placeholder features (zeros)")
print(f"   → Matches prediction worker expectations exactly!")
print()
...
# Placeholder features (already 0 from SQL)
X['referee_favorability_score'] = X['referee_favorability_score'].fillna(0)
X['look_ahead_pressure_score'] = X['look_ahead_pressure_score'].fillna(0)
X['matchup_history_score'] = X['matchup_history_score'].fillna(0)
X['momentum_score'] = X['momentum_score'].fillna(0)
...
# Hyperparameters (good defaults for tabular data)
params = {
    'max_depth': 6,
    'learning_rate': 0.1,
    'n_estimators': 200,
...
print(f"Training {params['n_estimators']} trees...")
...
model_id = f"xgboost_real_v3_25features_{datetime.now().strftime('%Y%m%d')}"
