#!/usr/bin/env python3
"""
Final Ensemble v8 - All Features Combined

Features:
- 25 base features (from ml_feature_store_v2)
- 6 Vegas features (points line, opening, movement, indicator, opponent history)
- 2 minutes/efficiency features (minutes_avg_last_10, ppm_avg_last_10)
= 33 total features

Models: XGBoost, LightGBM, CatBoost → Stacked Ensemble
...
print("=" * 80)
print(" FINAL ENSEMBLE V8 - ALL FEATURES COMBINED")
print("=" * 80)
...
print(f"\n{'Model':<20} {'MAE':>8} {'vs v6':>10} {'vs v7':>10} {'vs Mock':>10}")
print("-" * 60)
print(f"{'Mock v1':<20} {MOCK_MAE:>8.3f} {'+15.9%':>10} {'+23.7%':>10} {'--':>10}")
print(f"{'XGBoost v6':<20} {V6_MAE:>8.3f} {'--':>10} {'+6.7%':>10} {'-13.8%':>10}")
print(f"{'v7 Stacked Ens':<20} {V7_MAE:>8.3f} {'-6.3%':>10} {'--':>10} {'-19.1%':>10}")
print("-" * 60)
...
for name, data in sorted_results:
    ...
    vs_v7 = ((V7_MAE - mae) / V7_MAE) * 100
    vs_mock = ((MOCK_MAE - mae) / MOCK_MAE) * 100
    marker = " ***" if name == sorted_results[0][0] else ""
    print(f"{name:<20} {mae:>8.4f} {vs_v6:>+9.1f}% {vs_v7:>+9.1f}% {vs_mock:>+9.1f}%{marker}")
...
print(f"vs v6: {((V6_MAE - best_mae) / V6_MAE) * 100:+.1f}%")
print(f"vs v7: {((V7_MAE - best_mae) / V7_MAE) * 100:+.1f}%")
print(f"vs Mock: {((MOCK_MAE - best_mae) / MOCK_MAE) * 100:+.1f}%")
...
xgb_model.get_booster().save_model(str(MODEL_OUTPUT_DIR / f"xgboost_v8_33features_{timestamp}.json"))
lgb_model.booster_.save_model(str(MODEL_OUTPUT_DIR / f"lightgbm_v8_33features_{timestamp}.txt"))
cb_model.save_model(str(MODEL_OUTPUT_DIR / f"catboost_v8_33features_{timestamp}.cbm"))
...
metadata = {
    ...
    'baselines': {'v6': V6_MAE, 'v7': V7_MAE, 'mock': MOCK_MAE}
}
...
Features: {len(all_features)} (25 base + 4 Vegas + 2 opponent + 2 minutes/PPM)
...
Improvement:
  vs v6 (4.14):  {((V6_MAE - best_mae) / V6_MAE) * 100:+.1f}%
  vs v7 (3.88):  {((V7_MAE - best_mae) / V7_MAE) * 100:+.1f}%
  vs Mock (4.80): {((MOCK_MAE - best_mae) / MOCK_MAE) * 100:+.1f}%
""")
print("=" * 80)
