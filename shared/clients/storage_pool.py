"""
Google Cloud Storage Client Connection Pool

Provides thread-safe singleton pattern for GCS client reuse across the application.
Reduces connection overhead and resource usage.

Usage:
    from shared.clients.storage_pool import get_storage_client

    # Get cached client (or create if first time)
    client = get_storage_client(project_id="nba-props-platform")

    # Use client normally
    bucket = client.bucket("my-bucket")
    blob = bucket.blob("path/to/file.json")

Features:
- Thread-safe singleton pattern
- Lazy initialization (clients created on first use)
- Per-project client caching
- Automatic cleanup on application shutdown
- Compatible with all existing GCS code

Version: 1.0
Created: 2026-01-24
"""

import threading
import atexit
import logging
import os
from typing import Dict, Optional
from google.cloud import storage

logger = logging.getLogger(__name__)

# Global client cache (thread-safe)
_client_cache: Dict[str, storage.Client] = {}
_cache_lock = threading.Lock()


def get_storage_client(project_id: Optional[str] = None) -> storage.Client:
    """
    Get a cached GCS client for the specified project.

    Creates a new client on first call for a given project, then reuses it
    for all subsequent calls. Thread-safe for concurrent access.

    Args:
        project_id: GCP project ID (defaults to GCP_PROJECT_ID env var)

    Returns:
        storage.Client: Cached or newly created GCS client

    Example:
        # In any module:
        client = get_storage_client("nba-props-platform")

        # Same client instance will be returned on subsequent calls
        same_client = get_storage_client("nba-props-platform")
        assert client is same_client  # True!

    Thread Safety:
        Multiple threads can safely call this function concurrently.
        Each will receive the same client instance for a given project.

    Performance:
        - First call: ~100-300ms (creates client, authenticates)
        - Subsequent calls: <1ms (returns cached client)
    """
    # Default project from environment
    if project_id is None:
        project_id = os.environ.get('GCP_PROJECT_ID') or os.environ.get('GCP_PROJECT', 'nba-props-platform')

    cache_key = project_id

    # Fast path: client already exists (no lock needed for read)
    if cache_key in _client_cache:
        return _client_cache[cache_key]

    # Slow path: need to create client (acquire lock)
    with _cache_lock:
        # Double-check: another thread may have created it while we waited
        if cache_key in _client_cache:
            return _client_cache[cache_key]

        # Create new client
        logger.info(f"Creating new GCS client for project: {project_id}")

        client = storage.Client(project=project_id)

        _client_cache[cache_key] = client

        logger.info(
            f"GCS client created and cached. "
            f"Total cached clients: {len(_client_cache)}"
        )

        return client


def close_all_clients():
    """
    Close all cached GCS clients.

    Called automatically on application shutdown via atexit.
    Can also be called manually for cleanup (e.g., in tests).

    Note: After calling this, the next get_storage_client() call
    will create a new client.
    """
    with _cache_lock:
        client_count = len(_client_cache)

        if client_count == 0:
            return

        logger.info(f"Closing {client_count} cached GCS client(s)...")

        for cache_key, client in _client_cache.items():
            try:
                client.close()
                logger.debug(f"Closed GCS client: {cache_key}")
            except Exception as e:
                logger.warning(f"Error closing GCS client {cache_key}: {e}")

        _client_cache.clear()
        logger.info("All GCS clients closed")


def get_client_count() -> int:
    """
    Get the number of cached GCS clients.

    Useful for monitoring and debugging.

    Returns:
        int: Number of clients currently in the cache
    """
    return len(_client_cache)


def clear_cache():
    """
    Clear the client cache without closing clients.

    Warning: This leaves clients open but unreferenced. Only use in tests
    or when you know what you're doing. Prefer close_all_clients() instead.
    """
    with _cache_lock:
        _client_cache.clear()


# Register cleanup on application shutdown
atexit.register(close_all_clients)


# Backward compatibility: Allow direct Client creation
Client = storage.Client


if __name__ == "__main__":
    # Demo: Show connection pooling behavior
    import time

    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s'
    )

    print("Demo: GCS Connection Pooling\n")
    print("=" * 60)

    # First call - creates client
    print("\nFirst call to get_storage_client()...")
    start = time.time()
    client1 = get_storage_client("nba-props-platform")
    elapsed1 = (time.time() - start) * 1000
    print(f"Created client in {elapsed1:.2f}ms")

    # Second call - returns cached client
    print("\nSecond call to get_storage_client()...")
    start = time.time()
    client2 = get_storage_client("nba-props-platform")
    elapsed2 = (time.time() - start) * 1000
    print(f"Retrieved cached client in {elapsed2:.2f}ms")

    # Verify same instance
    print(f"\nSame client instance? {client1 is client2}")
    if elapsed2 > 0:
        print(f"Speedup: {elapsed1 / elapsed2:.0f}x faster")

    # Show cache stats
    print(f"\nCached clients: {get_client_count()}")

    # Cleanup
    print("\nClosing all clients...")
    close_all_clients()
    print(f"Cached clients after close: {get_client_count()}")
