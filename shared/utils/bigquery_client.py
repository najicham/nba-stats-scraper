...
# `pandas` is heavy (~50MB) and only used by query_to_dataframe(); imported
# lazily so `shared.utils` stays light enough for Cloud Functions that don't
# install pandas (e.g. backfill-pubsub-subscriber via parameter_resolver).
...
class BigQueryClient:
    ...
    def query_to_dataframe(self, query: str,
        ...
        """
        import pandas as pd  # lazy: only callers that use this method need pandas
