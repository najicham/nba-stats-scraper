...
# ESPN team abbreviations - MUST match the scraper's ESPN_TEAM_IDS
# ESPN uses different codes than NBA for some teams:
#   GS (not GSW), NO (not NOP), NY (not NYK), SA (not SAS), UTAH (not UTA)
ESPN_TEAM_IDS = {
    ...
    "SAC": 23,  "SA": 24,   "TOR": 28,  "UTAH": 26, "WAS": 27
}

# Mapping from NBA codes to ESPN codes (for convenience)
NBA_TO_ESPN = {
    ...
    def scrape_teams(self, team_list: Optional[List[str]] = None) -> Dict:
        ...
        Args:
            team_list: List of team abbreviations. If None, scrapes all 30 teams.
                       Accepts both NBA codes (GSW) and ESPN codes (GS).
        ...
        if team_list is None:
            ...
            # Convert NBA codes to ESPN codes if needed
            team_list = [NBA_TO_ESPN.get(t, t) for t in team_list]
