#!/usr/bin/env python3
"""
Production Training Script: LightGBM V1 MLB Pitcher Strikeouts Regressor
...
# LightGBM hyperparameters — conservative starting point matching CatBoost complexity
HYPERPARAMS = {
    'objective': 'regression',
    'metric': 'mae',
    'num_leaves': 15,          # ~2^depth where depth=4
    'learning_rate': 0.015,
    'n_estimators': 500,
    'reg_lambda': 10.0,        # L2 regularization (matches CatBoost l2_leaf_reg)
    'subsample': 0.8,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train LightGBM V1 MLB Pitcher Strikeouts Regressor"
    )
...
def train_model(X_train, y_train):
    ...
    # LightGBM needs NaN handled — it supports them natively
...
def main():
    ...
    print("=" * 70)
    print("  LightGBM V1 MLB Pitcher Strikeouts Regressor — Production Training")
    print("=" * 70)
    ...
    print(f"\nTraining LightGBM ({len(X_train):,} samples, {len(feature_cols)} features)...")
    ...
    model_name = f"lightgbm_mlb_v1_regressor_36f_{date_str}"

    model_path = output_path / f"{model_name}.txt"
