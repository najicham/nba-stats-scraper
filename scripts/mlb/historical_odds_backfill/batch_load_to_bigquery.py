#!/usr/bin/env python3
"""
MLB Historical Props Batch Loader - OPTIMIZED

Instead of loading each file individually (5 sec/file), this script:
1. Reads ALL files for multiple dates from GCS
2. Transforms all data in memory
3. Does a SINGLE batch insert to BigQuery per chunk

Performance: ~1000 rows/second vs ~3 rows/second for individual loads
...
Usage:
    ...
    python scripts/mlb/historical_odds_backfill/batch_load_to_bigquery.py --dry-run
"""
...
TABLE_ID = f'{PROJECT_ID}.mlb_raw.oddsa_pitcher_props'
...
class BatchLoader:
    """Optimized batch loader for historical MLB props."""
    ...
    def run(self) -> Dict:
        ...
        logger.info("=" * 70)
        logger.info("MLB HISTORICAL PROPS BATCH LOADER")
        logger.info("=" * 70)
        ...
        for i, game_date in enumerate(dates_to_process, 1):
            ...
            files_count, rows_count = self.process_date(game_date)
            ...
            if len(all_rows) >= BATCH_SIZE:
                ...
                strikeouts = sum(1 for r in all_rows if r.get('market_key') == 'pitcher_strikeouts')
                logger.info(f"  Batch written: {loaded} rows ({strikeouts} strikeouts)")
                all_rows = []
...
def main():
    parser = argparse.ArgumentParser(
        description='Batch load MLB historical props to BigQuery',
