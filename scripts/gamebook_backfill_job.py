...
Usage:
  ...
  ./bin/deployment/deploy_gamebook_job.sh
...
class NbaGamebookBackfillJob:
    ...
    def _print_final_summary(self, start_time: datetime):
        ...
        logger.info("   - Validate with: python scripts/validate_gamebook_data.py --recent")
        logger.info("   - Begin name mapping (Basketball Reference ↔ NBA Gamebook)")
        logger.info("   - Start processor development")


# Replace the main() function in scripts/gamebook_backfill_job.py with this:
