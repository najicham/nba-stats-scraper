...
def main():
    ...
    ORDER BY game_date
    """
    df_raw_player = execute_query(client, query, "1.1 - nba_raw.bdl_player_boxscores")
    ...
    FROM `{project_id}.nba_raw.bdl_team_boxscores`
    WHERE DATE(game_date) IN ('2026-01-19', '2026-01-20', '2026-01-21')
    GROUP BY DATE(game_date)
    ORDER BY game_date
    """
    df_raw_team = execute_query(client, query, "1.2 - nba_raw.bdl_team_boxscores")
    ...
    ORDER BY game_date
    """
    df_raw_games = execute_query(client, query, "1.3 - nba_raw.bdl_games")
    ...
    ORDER BY game_date
    """
    df_analytics_player = execute_query(client, query, "1.4 - nba_analytics.player_game_summary")
    ...
    SELECT
        ...
        COUNT(DISTINCT game_id) as unique_games,
        COUNT(DISTINCT team_id) as unique_teams
    FROM `{project_id}.nba_analytics.team_game_summary`
    WHERE DATE(game_date) IN ('2026-01-19', '2026-01-20', '2026-01-21')
    GROUP BY DATE(game_date)
    ORDER BY game_date
    """
    df_analytics_team = execute_query(client, query, "1.5 - nba_analytics.team_game_summary")
    ...
    """
    df_precompute = execute_query(client, query, "1.6 - nba_precompute.player_daily_cache")
    ...
    """
    df_predictions = execute_query(client, query, "1.7 - nba_predictions.player_prop_predictions")
    ...
    SELECT
        ...
        COUNT(DISTINCT game_id) as unique_games,
        COUNT(DISTINCT team_id) as unique_teams
    FROM `{project_id}.nba_predictions.team_predictions`
    WHERE prediction_date IN ('2026-01-19', '2026-01-20', '2026-01-21')
    GROUP BY prediction_date
    ORDER BY prediction_date
    """
    df_team_predictions = execute_query(client, query, "1.8 - nba_predictions.team_predictions")
    ...
    FROM `{project_id}.nba_predictions.player_prop_results`
    WHERE grading_date IN ('2026-01-19', '2026-01-20', '2026-01-21')
    GROUP BY grading_date
    ORDER BY grading_date
    """
    df_results = execute_query(client, query, "1.9 - nba_predictions.player_prop_results")
    ...
    ORDER BY game_date
    """
    execute_query(client, query, "2.1 - NULL values in nba_raw.bdl_player_boxscores")
    ...
    LIMIT 50
    """
    execute_query(client, query, "2.2 - Duplicate records in raw player boxscores")
    ...
    ORDER BY game_date
    """
    execute_query(client, query, "2.3 - Timestamp spread for raw player data")
    ...
    ORDER BY game_date
    """
    execute_query(client, query, "2.4 - Players with zero/NULL minutes (DNP)")
    ...
    INNER JOIN analytics_players a
        ON r.game_id = a.game_id AND r.player_id = a.player_id
    """
    execute_query(client, query, "3.1 - Jan 19: Raw vs Analytics discrepancy")
    ...
    SELECT
        ...
        r.reb,
        r.ast
    FROM `{project_id}.nba_raw.bdl_player_boxscores` r
    LEFT JOIN `{project_id}.nba_analytics.player_game_summary` a
    ...
    LIMIT 100
    """
    execute_query(client, query, "3.2 - Players in raw but NOT in analytics (Jan 19)")
    ...
    ORDER BY game_id
    """
    execute_query(client, query, "3.3 - Jan 20: Predictions breakdown by game")
    ...
    """
    execute_query(client, query, "3.4 - Jan 20: Predictions by prop type")
    ...
    FROM `{project_id}.nba_predictions.player_prop_predictions`
    """
    execute_query(client, query, "4.1 - Date range and total records for all tables")
    ...
    """
    execute_query(client, query, "4.2 - Date gaps in last 30 days")
    ...
    ORDER BY game_id
    """
    execute_query(client, query, "5.1 - Games scheduled on Jan 20, 2026")
    ...
    """
    execute_query(client, query, "5.2 - Game statuses by date")
    ...
    """
    execute_query(client, query, "5.3 - Postponed or cancelled games")
    ...
    for target_date in target_dates:
        ...
        raw_count = df_raw_player[df_raw_player['game_date'] == target_date]['record_count'].values
        analytics_count = df_analytics_player[df_analytics_player['game_date'] == target_date]['record_count'].values
        games_count = df_raw_games[df_raw_games['game_date'] == target_date]['record_count'].values
        pred_count = df_predictions[df_predictions['prediction_date'] == target_date]['record_count'].values
