#!/bin/bash
# File: monitoring/processing_gap_detection/deploy.sh
# Deploy Processing Gap Monitor to Cloud Run Jobs
...
echo "========================================="
echo "Processing Gap Monitor Deployment"
echo "========================================="
...
ENV_VARS="${ENV_VARS},BREVO_FROM_NAME=${BREVO_FROM_NAME:-Gap Detection System}"
ENV_VARS="${ENV_VARS},SLACK_ALERTS_ENABLED=${SLACK_ALERTS_ENABLED:-true}"
ENV_VARS="${ENV_VARS},SLACK_WEBHOOK_URL=${SLACK_WEBHOOK_URL}"
ENV_VARS="${ENV_VARS},SLACK_WEBHOOK_URL_ERROR=${SLACK_WEBHOOK_MONITORING_ERROR:-${SLACK_WEBHOOK_URL}}"
ENV_VARS="${ENV_VARS},SLACK_WEBHOOK_URL_WARNING=${SLACK_WEBHOOK_MONITORING_WARNING:-${SLACK_WEBHOOK_URL}}"
...
if [ $DEPLOY_STATUS -eq 0 ]; then
    ...
    echo "  Slack: $([ ! -z "${SLACK_WEBHOOK_MONITORING_ERROR}" ] && echo "#gap-monitoring" || echo "Using fallback")"
    echo ""
    echo "Manual execution:"
    echo "  gcloud run jobs execute ${JOB_NAME} --region=${REGION}"
    echo ""
    echo "Test with specific date:"
    echo "  gcloud run jobs execute ${JOB_NAME} --region=${REGION} \\"
    echo "    --args=\"--date=2025-08-31\""
    echo ""
    ...
    echo "To add scheduling later, run: ./setup-scheduler.sh"
    echo ""
else
    echo ""
